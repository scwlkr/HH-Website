...
test("detail normalization keeps all 35 Plan Your Home answers in exact tour order", () => {
  ...
  assert.equal(answers.length, 35);
  assert.deepEqual(
    answers.map(({ id }) => id),
...
test("detail UI exposes safe file/link actions and an accessible destructive confirmation", async () => {
  ...
  const query = within(view.container);
  assert.equal(query.getAllByText(/^Question \d+$/).length, 35);
