...
Plan Your Home phone prompt screens use one viewport-locked structure: a compact branded header, a concise zone/progress rail, an architectural context strip near 22% of the dynamic viewport, a task-first question surface, and a fixed Back/Next dock. The question owns most of the available height. Ordinary prompts do not scroll. Compound prompts become short substeps while retaining the existing question number, and the same bottom Next button advances both substeps and questions. Never show a second forward action inside the prompt. User-created collections and enlarged-text or onscreen-keyboard states may use contained scrolling as an accessibility escape hatch.
