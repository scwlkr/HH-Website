...
Plan Your Home phone prompt screens use one viewport-locked structure: a compact branded header, a concise zone/progress rail, an architectural context strip near 22% of the dynamic viewport, a task-first question surface, and a fixed Back/Next dock. The question owns most of the available height. Ordinary prompts do not scroll. Compound prompts become short substeps while retaining the existing question number, and the same bottom Next button advances both substeps and questions. Never show a second forward action inside the prompt. Completing a room saves in the background and advances directly into the next question; do not interrupt the walkthrough with saved-room, threshold, or checkpoint screens. User-created collections and enlarged-text or onscreen-keyboard states may use contained scrolling as an accessibility escape hatch.

The context strip uses exactly five sparse sketch families: front door and site; living, kitchen, and dining; bedrooms and bathrooms; utility, exterior, and outdoor; and design desk and review. Each family stays visually fixed across its related prompts, with no moving highlights, captions, people, or figure-like decorations. Keep the artwork atmospheric and professional: few strokes, restrained washes, no labels inside the scene, no customer-specific reconfiguration, and no decorative objects that compete with the task.
...
Forms are functional, readable, and hard to misuse. Inputs use white or raised surfaces, clear labels, strong enough contrast, and visible focus rings. Inquiry progress must remain understandable to screen readers. Keep technical privacy, storage, and retention explanations on the privacy page; forms use concise action-specific consent and a quiet policy link.

Plan Your Home uses the `plan-home-*` component tokens as a single mobile app frame. The context strip is a shallow architectural view, not a backdrop behind controls. Choices are large, flat, and immediately selectable. Back and the applicable progression action remain fixed throughout the planning flow. Dense content is divided into focused substeps instead of compressed into a small inner scroller. On desktop, the review cover is a compact orientation band that keeps the brief index and first review section visible in the initial viewport. The desktop brief scrolls vertically inside the app frame while the header and brief index remain sticky; the phone review stays paged with fixed Back and Next actions. The isolated owner-review route adds compact header Back, Next, and Reset controls that bypass validation and traverse questions, the contact gate, and final-brief pages for visual review only; they never alter the customer-route interaction contract.
