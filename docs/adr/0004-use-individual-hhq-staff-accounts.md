...
- Status: Accepted; live in production, Google and email-code sign-in verified
- Date: 2026-09-22
...
Production explicitly selects WorkOS; the shared Firebase login remains available only through the provider switch for local development and deliberate rollback. Historical actions performed through that shared identity cannot be attributed to individual staff; individual accounts do not by themselves add an audit-log product or new activity-tracking features.
